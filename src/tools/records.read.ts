...
export async function handleListRecords(
  ...
  try {
    ...
    if (ids && ids.length > 0) {
      ...
      const response: Record<string, unknown> = { items, total: items.length, count: items.length, nextCursor: null };
    ...
    const response: Record<string, unknown> = { items, total: res.total, count: items.length, nextCursor };
    if (ctxMap) response['_fieldContext'] = ctxMap;
    return ok(response);
  } catch (e) {
...
export async function handleSearchRecords(
  ...
  try {
    ...
    const response: Record<string, unknown> = { items, total: res.total, count: items.length, query, nextCursor };
    if (ctxMap) response['_fieldContext'] = ctxMap;
    return ok(response);
  } catch (e) {
    return { content: [{ type: 'text', text: JSON.stringify({ error: toErrorResponse(e) }, null, 2) }], isError: true };
